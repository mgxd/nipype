...
class SlicerInputSpec(FSLCommandInputSpec):
    in_file = File(
        exists=True, position=1, argstr="%s", mandatory=True, desc="input volume"
    )
    image_edges = File(
        exists=True,
        position=2,
...
class ConvertXFMInputSpec(FSLCommandInputSpec):
    ...
    invert_xfm = traits.Bool(
        argstr="-inverse", position=-3, xor=_options, desc="invert input transformation"
    )
...
class InvWarpInputSpec(FSLCommandInputSpec):
    warp = File(
        ...
        mandatory=True,
        desc=(
            "Name of file containing warp-coefficients/fields. This "
    ...
    reference = File(
        ...
        mandatory=True,
        desc=(
            "Name of a file in target space. Note that the "
    ...
    inverse_warp = File(
        ...
        desc=(
            "Name of output file, containing warps that are "
    ...
    absolute = traits.Bool(
        ...
        xor=["relative"],
        desc=(
            "If set it indicates that the warps in --warp"
    ...
    niter = traits.Int(
        argstr="--niter=%d",
        desc=(
            "Determines how many iterations of the "
...
class Complex(FSLCommand):
    ...
    def _list_outputs(self):
        outputs = self.output_spec().get()
        if (
            self.inputs.complex_cartesian
            or self.inputs.complex_polar
            or self.inputs.complex_split
            or self.inputs.complex_merge
        ):
...
class WarpUtilsInputSpec(FSLCommandInputSpec):
    ...
    out_format = traits.Enum(
        ...
        argstr="--outformat=%s",
        desc=(
            "Specifies the output format. If set to field (default) "
    ...
    warp_resolution = traits.Tuple(
        ...
        desc=(
            "Specifies the resolution/knot-spacing of the splines pertaining"
...
class ConvertWarpInputSpec(FSLCommandInputSpec):
    ...
    out_file = File(
        ...
        output_name="out_file",
        desc=(
            "Name of output file, containing warps that are the combination "
